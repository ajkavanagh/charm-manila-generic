...
class ManilaBasicDeployment(OpenStackAmuletDeployment):
    ...
    def test_200_manila_identity_relation(self):
        ...
        manila_admin_endpoint = "http://%s:9312" % (manila_ip)
        manila_endpoint = "http://%s:9311" % (manila_ip)

        expected = {
            'admin_url': manila_admin_endpoint,
            'internal_url': manila_endpoint,
            'private-address': manila_ip,
            'public_url': manila_endpoint,
            'region': 'RegionOne',
            'service': 'manila',
        }
